...
def get_ncbi_taxon_id(bundle: Bundle):
    ...
    if len(cellSuspensions) != 1:
        raise UnsupportedOrganismException('Multiple cell suspensions detected in bundle.')
    ...
    if any([taxon_id != first_taxon_id for taxon_id in cellSuspension.ncbi_taxon_id]):
        raise UnsupportedOrganismException('Multiple distinct species detected in bundle.')
